...
def create_violin_plots(folder_path, column_name=None):
    ...
    plt.title('Claude 3.5 Sonnet', fontsize=48, fontweight='bold', pad=20)
    
    # Adjust layout to prevent label cutoff
    plt.tight_layout()
    
    # Save the plot to Desktop
    output_filename = os.path.expanduser('~/Desktop/secondary_violin_plot_comparison_claude.png')
    plt.savefig(output_filename)
    plt.close()
    print(f"Created violin plot: {output_filename}")

def main():
    folder_path = os.path.expanduser("~/Desktop/SA_2_claude")
